package arthur_cattaneo_zavadski.aula20200821.repository;
...
public class ImóvelRepository {
	...
	public List<Imóvel> findAll() throws SQLException {
		List<Imóvel> all = new ArrayList<>();
		PreparedStatement psSelect = connectionManager.prepareStatement("select id, descricao, metragem from imovel");
		ResultSet rsSelect = psSelect.executeQuery();
		try {
			while (rsSelect.next()) {
				Imóvel imóvel = new Imóvel(
						rsSelect.getLong("id"), 
						rsSelect.getString("descricao"), 
						rsSelect.getDouble("metragem"));
				all.add(imóvel);
